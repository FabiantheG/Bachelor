...
def xgboost(cur_list, hedge_logreturn, factors, window=60, duration=1, xgb_factor = None,xgb_predictors = None,pred = ['ted', 'volatility', 'afd', 'commodity']):
...
def xgboost_mean(cur_list, hedge_logreturn, factors, window=60, duration=1, xgb_factor = None,xgb_predictors = None):
    ...
    while t <= len(first_col) - duration:
        ...
        expected_carry = factor_shifted_window['carry_shifted'].mean()


        expected_dollar = factor_shifted_window['dollar_shifted'].mean()
...
def xgboost_tminus1(cur_list, hedge_logreturn, factors, window=60, duration=1, xgb_factor = None,xgb_predictors = None):
    ...
    while t <= len(first_col) - duration:
        ...
        expected_carry = factor_shifted_window['carry_shifted'].iloc[-1]


        expected_dollar = factor_shifted_window['dollar_shifted'].iloc[-1]
...
def xgboost_vola(cur_list, hedge_logreturn, factors,vola, base, window=60, duration=1, xgb_factor = None,xgb_predictors = None,pred = ['ted', 'volatility', 'afd', 'commodity']):
    ...
    # Join everything into one aligned DataFrame
    f = pd.concat([factors, hedge_logreturn, vola], axis=1, join='inner')

    columns = list(vola.columns)
    ...
    for cur in cur_list:
        ...
        pred = factors[['dollar','carry','fxvolatility'+base+cur]]
        t = window
        while t <= len(hedge_cur) - duration:
            hedge_window = hedge_cur.iloc[t - window:t]
            factor_window = pred.iloc[t - window:t]
